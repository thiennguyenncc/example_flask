...
use Bachelor\Domain\NotificationManagement\Notification\EligibleReceivers\CompletedDatingHasFBByPartnerNoDisplayReport;
...
class NoUpdatedDatingReport extends AbstractNotificationSenderCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'notification:dating-report-no-updated {gender}';
     ...
     * @var string
     */
    protected $description = 'Dating report dont updated in sunday or thursday on 9am';
    ...
    public function __construct(
        ...
        CompletedDatingHasFBByPartnerNoDisplayReport $completedDatingHasFBByPartnerNoDisplayReport
    ) {
        $this->feedbackRepository = $feedbackRepository;
        $this->datingReportRepository = $datingReportRepository;
        $this->datingReportDomainService = $datingReportDomainService;
        $this->eligibleReceiver = $completedDatingHasFBByPartnerNoDisplayReport;
    ...
    protected function getKey(): ?string
    {
        if ($this->getEligibleGender() == UserGender::Female) {
            return config('notification_keys.dating_report_no_updated_for_female_users');
        } elseif ($this->getEligibleGender() == UserGender::Male) {
            return config('notification_keys.dating_report_no_updated_for_male_users');
        }
    ...
    {
        $notification->mapVariable('required_review_number_for_next_renewal',
            $this->variableMapDatas[$user->getId()] ?? config('constants.report_generate_frequency_fb'));
