...
class ReactivatedNotification
{
    ...
    /**
     * ApprovedNotification constructor.
    ...
    public function handle(GetReactivatedFemaleOrPaidMale $event)
    {
        /** @var User $user */
        $user = $event->user;

        if ($user->getGender() == UserGender::Male) {
            $key = config('notification_keys.reactivated_for_male_user');
            ...
            $key = config('notification_keys.reactivated_for_female_user');
            $notification = $this->notificationRepository->getByKey($key);
