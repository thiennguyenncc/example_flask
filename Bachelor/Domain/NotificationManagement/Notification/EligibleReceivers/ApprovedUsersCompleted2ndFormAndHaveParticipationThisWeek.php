...
class ApprovedUsersCompleted2ndFormAndHaveParticipationThisWeek extends AbstractEligibleReceiver
{
    ...
    public function retrieve(): Collection
    {
        ...
        return $userList->filter(function ($user) use ($participants) {
            /* @var User $user */
            $filteredParticipants = $participants->filter(function ($participant) use ($user) {
                /* @var ParticipantMainMatch $participant */
                return $participant->getUserId() == $user->getId();
            });
            if ($filteredParticipants->count()) {
